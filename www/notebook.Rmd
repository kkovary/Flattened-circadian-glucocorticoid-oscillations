...
gene.df <- clusterProfiler::bitr(on_diag$ext_gene_bat, 
                                 fromType = 'SYMBOL', 
                                 toType = 'ENTREZID', 
                                 OrgDb = org.Mm.eg.db)

    
universe <- clusterProfiler::bitr(all_combined$ext_gene_bat, 
                                  fromType = 'SYMBOL', 
                                  toType = 'ENTREZID', 
                                  OrgDb = org.Mm.eg.db)
...
subsetFC <- all_combined %>% dplyr::rename(GeneName = ext_gene_bat)
...
gene.df <- clusterProfiler::bitr(off_diag$ext_gene_bat, 
                                 fromType = 'SYMBOL', 
                                 toType = 'ENTREZID', 
                                 OrgDb = org.Mm.eg.db)
...
all_combined %>% filter(ext_gene %in% gabp1_ets1,
                        fc > 2,
                        lrt_qval < 0.05) %>%
  ggplot(., 
         aes(y = -log10(lrt_qval), 

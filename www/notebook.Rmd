...
eight_e <- categories %>% 
  ...
  dplyr::filter(category %in% c("lipid metabolism", "Circadian rhythm")) %>%
  left_join(joined_wat_gene_results, by = "target_id") %>%
  left_join(joined_bat_gene_results, by = "target_id") %>%
...
```{r fig_s2, fig.height=8, fig.width=8}
# Sup Fig 2 A
...
(s2_a + plot_spacer() + plot_spacer()) / (s2_b + s2_c) +
...
(s3_a + plot_spacer() + plot_spacer()) / (s3_b + s3_c) +
  plot_layout(heights = c(0.5,1)) +
  plot_annotation(tag_levels = "A",
                  title = 'Fig. S3') &
  theme(text = element_text(size=8)) 
